...
describe 'ircd-hybrid::build' do
	...
	it 'should configure with ssl if requested' do
		expect(chef_run_ssl).to run_bash("configure")
			.with(cwd: build_dir, user: user, group: group, code: "#{configure_script} --enable-openssl")
	end

	it 'should build' do
		expect(chef_run).to run_bash("make").with(cwd: build_dir, user: user, group: group)
	end
	...
	it 'should install' do
		expect(chef_run).to run_bash("make install").with(cwd: build_dir, user: user, group: group)
	end
end
